(() => {
  ...
  var Mousewheel = ({ swiper, extendParams, on, emit }) => {
    ...
    function handle(event) {
      ...
      if (params.invert)
        delta = -delta;
      let positions = swiper.getTranslate() + delta * ((_a = params.sensitivity) != null ? _a : 1);
      ...
      if (!swiper.params.freeMode || !(freeModeParams == null ? void 0 : freeModeParams.enabled)) {
        ...
        if (!ignoreWheelEvents) {
          ...
          let position = swiper.getTranslate() + delta * ((_b = params.sensitivity) != null ? _b : 1);
  ...
  var FreeMode = ({ swiper, extendParams, emit, once }) => {
    ...
    function onTouchMove2() {
      ...
      if (data.velocities.length === 0) {
        data.velocities.push({
          ...
          time: (_a = data.touchStartTime) != null ? _a : now()
        });
      }
      data.velocities.push({
    ...
    function onTouchEnd2({ currentPos }) {
      ...
      const touchEndTime = now();
      const timeDiff = touchEndTime - ((_a = data.touchStartTime) != null ? _a : touchEndTime);
      ...
      if (freeModeParams.momentum) {
        ...
        if (newPosition < swiper.maxTranslate()) {
          ...
          if (Math.abs(snapGrid[nextSlide] - newPosition) < Math.abs(((_b = snapGrid[nextSlide - 1]) != null ? _b : snapGrid[nextSlide]) - newPosition) || swiper.swipeDirection === "next") {
  ...
  // resources/tpl/default/modules/home-carousels.js
